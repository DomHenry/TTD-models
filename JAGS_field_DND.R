...
#   JAGS_field_DND.R
#
#   JAGS formulation for a hierarchical multi-species detection/non-detection occupancy 
...
cat("
    ...
    for(k in 1:n_spp){                                     # Loop over species 
      ...
      lp[k] ~ dnorm(mu.lp, tau.lp)                         # Detection probability intercept
    ...
    for(k in 1:n_spp){                                                    # Loop over species
      for (i in 1:n_pen) {                                                # Loop over sites    
        for (j in 1:rep_vec[i]){                                          # Loop over site survey replicates    
          logit(p[i,j,k]) <-lp[k] +                                            
